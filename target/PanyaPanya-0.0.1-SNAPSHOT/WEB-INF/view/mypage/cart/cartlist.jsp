...
<c:import url="/WEB-INF/view/include/top_menu.jsp" />
	...
	<div class="container" style="margin-top: 30px; margin-bottom: 30px;" align="center">
		...
		<table class="table table-hover" style="width: 100%; margin: auto; border-bottom: 1px solid #D5D5D5;">
			...
			<tbody>
				<c:choose>
					<%-- 장바구니에 물건이 있다면 In case that there are items in user's cart. --%>
					<c:when test="${cartList != null}">
						<c:forEach items="${cartList}" var="vo" varStatus="status">
							<tr>
								...
								<td><a href="${root }product/product_detail?product_idx=${vo.product_idx}">${vo.product_name}</a></td>
								<td style="text-align: center;"><fmt:formatNumber value="${vo.price }" pattern="###,###,###" />원</td>
								<td style="text-align: center;">
									<%-- Functions related to quantity change --%>
								...
								<td style="text-align: center;"><fmt:formatNumber value="${vo.price*vo.amount }" pattern="###,###,###" />원</td>
								<td style="text-align: center;"><a href="${root }mypage/cart/cartlist/delete?cart_idx=${vo.cart_idx}">삭제하기</a></td>
							</tr>
						</c:forEach>
					</c:when>
					<c:otherwise>
		...
		<%-- 주문정보입력 부분 The Section about entering Order information --%>
		<table class="table table-hover" style="width: 100%; margin: auto; border-bottom: 1px solid #D5D5D5;">
			<thead>
				<tr>
					<th style="text-align: center;" colspan="7">
						<div class="text-right">
							...
							배송비: 
							<c:choose>
								<%-- 장바구니에 물건이 들어있는 경우  In case that there is a goods in user cart --%>
								<c:when test="${cartList != null}">
									3,000 원<br>
								</c:when>
								<c:otherwise>0 원<br></c:otherwise>
							</c:choose>
							결제금액:
							<c:choose>
								<c:when test="${cartList != null}">
									<fmt:formatNumber value="${paymentMoney+3000}" pattern="###,###,###" /> 원
								</c:when>
			...
			<tbody>
				<tr class="sender">
					...
					<td colspan="3"><input type="text" style="width: 150px;" name="sender_name" id="sender_name" required="required" /></td>
				</tr>
				<tr class="sender">
					<td colspan="2"></td>
					<td colspan="2">발송인의 연락처(Tel):</td>
					<td colspan="3"><input type="tel" style="width: 150px;" name="sender_tel" id="sender_tel" required="required" /></td>
				</tr>
				<tr class="sender">
					<td colspan="2"></td>
				...
				<tr class="recipient">
					...
					<td colspan="3"><input type="text" style="width: 150px;" name="recipient_name" required="required" /></td>
				</tr>
				<tr class="recipient">
					<td colspan="2"></td>
					<td colspan="2">수령인의 연락처(Tel):</td>
					<td colspan="3"><input type="tel" style="width: 150px;" name="recipient_tel" id="recipient_tel" required="required" /></td>
				</tr>
				<tr class="recipient">
					<td colspan="2"></td>
				...
				<tr>
					<td colspan="2"></td>
					<td colspan="2">메모:</td>
					<td colspan="3"><textarea style="width: 600px; resize: none;" name="memo" placeholder="주문결제 또는 배송관련하여 남기실 말씀이 있으시면 여기에 남겨주세요!"></textarea></td>
				</tr>
				<tr>
					<td colspan="2"></td>
		...
		<br>
		<c:choose>
			<%--장바구니에 물건이 들어있는 경우. In case that There is items in the user's cart. --%>
			<c:when test="${cartList != null}">
