...
<c:import url="/WEB-INF/view/include/top_menu.jsp" />
	<div class="container text-center" style="margin-top: 50px; margin-bottom: 50px">
		...
		<c:choose>
				...
				<c:forEach items="${orderPaymentList}" var="orderPaymentList">
					<table style="border: 1px solid lightgray; width: 100%;">
						...
						<tr>
							...
							<td>
								${orderPaymentList.sender_name}<br>
								${orderPaymentList.sender_tel}<br>
								${orderPaymentList.sender_address}&nbsp;&nbsp;&nbsp;&nbsp;
								(우편번호: ${orderPaymentList.sender_postcode})
							</td>
						</tr>
						<tr>
							<th>
							...
							<td>
								${orderPaymentList.recipient_name}<br>
								${orderPaymentList.recipient_tel}<br>
								${orderPaymentList.recipient_address}&nbsp;&nbsp;&nbsp;&nbsp;
								(우편번호: ${orderPaymentList.recipient_postcode})
							</td>
						</tr>
						<tr>
							<th>메모(남기는 말씀)</th>
						...
						<tr>
							...
							<td>
								...
								(물품금액의 합계<fmt:formatNumber pattern="###,###,###" value="${orderPaymentList.paymentPrice-3000 }" /> 원 + 배송비 3,000 원)
							</td>
						</tr>
						<tr>
							<th>결제수단</th>
						...
						<tr>
							<th>배송상황</th>
							<td>
								<strong>${orderPaymentList.circumstance}</strong>
								<p class="attention">
									※결제가 완료된 후 배송준비 전일경우 100%환불이 가능하며, 배송준비후에는 반품접수가 이루어집니다.<br>
									환불이나 반품을 원하실 경우, 신속하게 유선(Tel. 010-5824-9508)으로 연락바랍니다.
								</p>
							</td>
						</tr>
					</table>
			...
			<c:otherwise>
				<p>${loginMemberDTO.member_name }고객님께서는아직 주문·결제를 완료하신 상품내역이 없습니다.</p>
			</c:otherwise>
		</c:choose>
	</div>
