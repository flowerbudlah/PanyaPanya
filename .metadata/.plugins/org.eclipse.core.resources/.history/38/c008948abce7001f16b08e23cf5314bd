	...
	function deleteProductReReply(){
		...
		if(yn){
			$.ajax({
				url		:	"${root}product/product_detail/deleteProductReply",
				type	:	"POST",
				data	:	{ product_rereply_idx : product_rereply_idx },
				dataType	:	"JSON",
				success  : function(obj) {
						...
						<c:if test="${productReReply.product_reply_idx == productReply.product_reply_idx}">
							<li>
								...
								<div class="reply_writer">
									...
									대댓글의 작성자: ${productReReply.product_rereplyer_name}
								</div>
								<div class="reply_date">대댓글 작성 일시: <fmt:formatDate value="${productReReply.regdate}" pattern="yyyy-MM-dd(E) HH:mm:ss"/>
...
<c:choose>
	...
	<%--관리자가 로그인을 한 경우(공지사항 게시판의 경우는 관리자만 사용가능하다. ) --%>
