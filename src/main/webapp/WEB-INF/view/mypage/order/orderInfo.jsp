...
h4{font-family: 'Single Day', cursive;}
				...
				<c:forEach items="${orderPaymentList}" var="orderPaymentList">
					<table style="border: 1px solid lightgray; width: 100%;">
						...
						<tr>
							<th>결제일시</th>
							<td>
								<fmt:formatDate pattern="yyyy-MM-dd(E) hh:mm:ss" value="${orderPaymentList.order_date}" />
							</td>
						</tr>
						<tr>
							<th>발송인의 이름<br>발송인의 연락처<br>발송인의 주소</th>
							<td>${orderPaymentList.sender_name}<br>${orderPaymentList.sender_tel}
								<br>${orderPaymentList.sender_address}&nbsp;&nbsp;&nbsp;&nbsp;
								(우편번호: ${orderPaymentList.sender_postcode})
							</td>
						</tr>
						<tr>
							<th>수령인의 이름<br>수령인의 연락처<br>수령인의 주소
							</th>
						...
						<tr>
							<th>결제금액</th>
							<td><strong>총 <fmt:formatNumber
										pattern="###,###,###"
										value="${orderPaymentList.paymentPrice }" />원
							</strong> (물품금액의 합계<fmt:formatNumber pattern="###,###,###"
									value="${orderPaymentList.paymentPrice-3000 }" /> 원 + 배송비
								3,000 원)</td>
						</tr>
						<tr>
							<th>결제수단</th>
						...
						<tr>
							...
							<td><strong>${orderPaymentList.circumstance}</strong>
								<p class="attention">
									※결제가 완료된 후 배송준비 전일경우 100%환불이 가능하며, 배송준비후에는 반품접수가 이루어집니다. <br>환불이나
									반품을 원하실 경우, 신속하게 유선(Tel. 02-313-1132, 010-5824-9508)으로 연락바랍니다.
								</p>
							</td>
						</tr>
					</table>
