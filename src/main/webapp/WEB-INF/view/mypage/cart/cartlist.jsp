...
<body>
	...
	<div class="container" style="margin-top: 30px; margin-bottom: 30px;" align="center">
		<!--본문내용-->
		<c:set value="${cartMap.cartList}" var="cartList" />
		<h5>
			<strong>${loginMemberDTO.member_name }님의 장바구니 </strong>
		</h5>
		<table class="table table-hover" style="width: 100%; margin: auto; border-bottom: 1px solid #D5D5D5;">
			...
			<tbody>
				<c:choose>
					...
					<c:otherwise>
						<tr>
							<td colspan="7"><p>장바구니에 내역이 없습니다.</p></td>
						</tr>
					</c:otherwise>
				</c:choose>
			</tbody>
		...
		<table class="table table-hover"
			style="width: 100%; margin: auto; border-bottom: 1px solid #D5D5D5;">
			<thead>
				<tr>
					<th style="text-align: center;" colspan="7">
						<div class="text-right">
							총 합계:
							<fmt:formatNumber value="${paymentMoney }" pattern="###,###,###" />원<br>
							배송비:
							<c:choose>
								<c:when test="${cartList != null}">
									<%--장바구니에 물건이 들어있다. --%>3,000 </c:when>
							...
							<c:choose>
								<c:when test="${cartList != null}">
									<%--장바구니에 물건이 들어있다. --%>
									<fmt:formatNumber value="${paymentMoney+3000}"
										pattern="###,###,###" />
								</c:when>
								<c:otherwise>0</c:otherwise>
							</c:choose>
			...
			<form action='${root }mypage/order' method='post' autocomplete="off">
				...
				<tbody>
					<tr class="sender">
						<td colspan="2"><input type="hidden" name="paymentPrice"
							id="paymentPrice" value="${paymentMoney+3000}" /></td>
						<td colspan="2">발송인의 이름:</td>
						<td colspan="3"><input type="text" style="width: 150px;"
							name="sender_name" id="sender_name" required="required" /></td>
					</tr>
					<tr class="sender">
						<td colspan="2"></td>
						<td colspan="2">발송인의 연락처(Tel):</td>
						<td colspan="3"><input type="tel" style="width: 150px;"
							name="sender_tel" id="sender_tel" required="required" /></td>
					</tr>
					<tr class="sender">
						<td colspan="2"></td>
						<td colspan="2">발송인의 주소:</td>
						<td colspan="3"><input type="text" name="sender_postcode"
							id="postcode" placeholder="우편번호" style="width: 150px;"> <input
							type="button" class="btn btn-primary btn-sm" id="searchAdd"
							value="우편번호 찾기"><br> <input type="text"
							name="sender_address" placeholder="상세주소" id="roadAddress"
					...
					<tr class="recipient">
						...
						<td colspan="3"><input type="text" style="width: 150px;"
							name="recipient_name" required="required" /></td>
					</tr>
					<tr class="recipient">
						<td colspan="2"></td>
						<td colspan="2">수령인의 연락처(Tel):</td>
						<td colspan="3"><input type="tel" style="width: 150px;"
							name="recipient_tel" id="recipient_tel" required="required" /></td>
					</tr>
					<tr class="recipient">
						<td colspan="2"></td>
						<td colspan="2">수령인의 주소:</td>
						<td colspan="3"><input type="text" name="recipient_postcode"
							id="postcode2" placeholder="우편번호" style="width: 150px;">
							<input type="button" id="searchAdd2" value="우편번호 찾기"
							class="btn btn-info btn-sm"><br> <input type="text"
							name="recipient_address" placeholder="상세주소" id="roadAddress2"
							style="width: 600px;" /></td>
					</tr>
					<tr>
						<td colspan="2"></td>
						<td colspan="2">메모:</td>
						<td colspan="3"><textarea style="width: 600px; resize: none;"
								name="memo" placeholder="주문결제 또는 배송관련하여 남기실 말씀이 있으시면 여기에 남겨주세요!"></textarea>
						</td>
					</tr>
					<tr>
						<td colspan="2"></td>
						<td colspan="2">결제방법</td>
						<td colspan="3"><input type="radio" name="payment_method"
							value="신용카드">신용카드<br> <input type="radio"
							name="payment_method" value="계좌이체">계좌이체</td>
					</tr>
